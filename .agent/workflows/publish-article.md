...
2. **Select Topic (Brand-Aware)**:
   - Read `src/dna.config.json` to find the `strategy_path` (e.g., `libs/brain/strategies/kominka_strategy.md`).
   - Read the Strategy File specified in `strategy_path`.
   - Select a topic from the list corresponding to `[CATEGORY]`. Define `[TOPIC]` and `[TOPIC_SLUG]`.
...
1. **Context Analysis (Crucial)**:
   ...
   - _Instruction to Orchestrator_: Do not just use specific generic terms. If the text describes "BGC night view", generate a night view. If it describes "Luxury Condo Interior", generate an interior.
2. Generate Image 1:
...
# Step 8: Batch Video Processing (Premium Workflow)

// turbo

1. **Crawl and Process Pending Projects**:
   - Run `node scripts/batch-processor.js`.
   - **Goal**: Automatically detect new clips in project folders, generate captions, and render videos for previously created articles.
   - This ensures that manual video clips uploaded since the last run are processed into final videos.
