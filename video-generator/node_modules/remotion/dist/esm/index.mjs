...
var AbsoluteFillRefForwarding = (props, ref) => {
  ...
  const actualStyle = useMemo(() => {
    return {
      position: "absolute",
      top: hasTailwindClassName(other.className, ["top-", "inset-"]) ? undefined : 0,
      left: hasTailwindClassName(other.className, ["left-", "inset-"]) ? undefined : 0,
      right: hasTailwindClassName(other.className, ["right-", "inset-"]) ? undefined : 0,
      bottom: hasTailwindClassName(other.className, ["bottom-", "inset-"]) ? undefined : 0,
      width: hasTailwindClassName(other.className, ["w-"]) ? undefined : "100%",
      height: hasTailwindClassName(other.className, ["h-"]) ? undefined : "100%",
...
var useMediaInTimeline = ({
  ...
  const videoConfig = useVideoConfig();
  const { rootId, audioAndVideoTags } = useContext15(TimelineContext);
  ...
  const { imperativePlaying } = useContext15(TimelineContext);
  const startsAt = useMediaStartsAt();
...
var BufferingContextReact = React13.createContext(null);
...
import React24, { createContext as createContext18 } from "react";
var CurrentScaleContext = React24.createContext(null);
...
var useCurrentScale = (options) => {
  const hasContext = React24.useContext(CurrentScaleContext);
  const zoomContext = React24.useContext(PreviewSizeContext);
...
import React25, { useMemo as useMemo25 } from "react";
import { jsx as jsx27 } from "react/jsx-runtime";
function useRemotionContexts() {
  const compositionManagerCtx = React25.useContext(CompositionManager);
  const timelineContext = React25.useContext(TimelineContext);
  const setTimelineContext = React25.useContext(SetTimelineContext);
  const sequenceContext = React25.useContext(SequenceContext);
  const nonceContext = React25.useContext(NonceContext);
  const canUseRemotionHooksContext = React25.useContext(CanUseRemotionHooks);
  const preloadContext = React25.useContext(PreloadContext);
  const resolveCompositionContext = React25.useContext(ResolveCompositionContext);
  const renderAssetManagerContext = React25.useContext(RenderAssetManager);
  const sequenceManagerContext = React25.useContext(SequenceManager);
  const bufferManagerContext = React25.useContext(BufferingContextReact);
  const logLevelContext = React25.useContext(LogLevelContext);
  return useMemo25(() => ({
...
var flattenChildren = (children) => {
  ...
  return childrenArray.reduce((flatChildren, child) => {
    if (child.type === React26.Fragment) {
...
import React27, { createContext as createContext19 } from "react";
...
var useRequireToBeInsideSeries = () => {
  const isInsideSeries = React27.useContext(IsInsideSeriesContext);
  if (!isInsideSeries) {
...
var Still = (props2) => {
  ...
  return React29.createElement(Composition, newProps);
};
