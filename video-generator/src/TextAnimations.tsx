...
export const FadeText: React.FC<FadeTextProps> = ({ text, durationFrames, sceneStartFrame = 0, style }) => {
...
export const TypewriterText: React.FC<TypewriterTextProps> = ({ text, durationFrames, sceneStartFrame = 0, style }) => {
  const frame = useCurrentFrame();
  const relativeFrame = frame - sceneStartFrame;
  ...
  const cursor = relativeFrame % 30 < 15 ? '|' : '';
...
export const WordPopupText: React.FC<WordPopupTextProps> = ({ text, durationFrames, sceneStartFrame = 0, style }) => {
  const frame = useCurrentFrame();
  const relativeFrame = frame - sceneStartFrame;
...
export const CountUpText: React.FC<CountUpTextProps> = ({ text, durationFrames, sceneStartFrame = 0, style }) => {
  const frame = useCurrentFrame();
  const relativeFrame = frame - sceneStartFrame;
...
export const SlideUpText: React.FC<SlideUpTextProps> = ({ text, durationFrames, sceneStartFrame = 0, style }) => {
  const frame = useCurrentFrame();
  const relativeFrame = frame - sceneStartFrame;
...
interface SyncedCaptionProps {
  captions: Caption[];
  sceneStartFrame: number;
  sceneDurationSec: number;  // シーン全体の長さ（フォールバック用）
  fallbackText?: string;     // キャプションがない区間で表示するテキスト
...
export const SyncedCaption: React.FC<SyncedCaptionProps> = ({
  ...
  if (activeCaptionIndex !== -1) {
    ...
    displayText = activeCaption.text;
    const captionDuration = activeCaption.end_sec - activeCaption.start_sec;
    captionProgress = (timeInSceneSec - activeCaption.start_sec) / captionDuration;
  } else {
    // フォールバック：キャプションがない区間
    ...
    const lastCaptionEndSec = lastCaption?.end_sec || 0;

    if (timeInSceneSec >= lastCaptionEndSec && timeInSceneSec < sceneDurationSec) {
      // キャプション終了後〜シーン終了まで：最後のキャプションを継続表示
      if (lastCaption) {
        displayText = lastCaption.text;
        // フェードを維持（切れないように）
        captionProgress = 0.5; // 中間状態を維持
      } else if (fallbackText) {
        displayText = fallbackText;
        captionProgress = 0.5;
      }
    } else if (timeInSceneSec < (captions[0]?.start_sec || 0)) {
      // シーン開始〜最初のキャプションまで：何も表示しない or fallbackText
      if (fallbackText) {
        displayText = fallbackText;
        captionProgress = 0.5;
      } else {
      ...
      // キャプション間のギャップ：前のキャプションを継続
      const prevCaption = captions.find((cap, idx) => {
        const nextCap = captions[idx + 1];
        return cap.end_sec <= timeInSceneSec && (!nextCap || nextCap.start_sec > timeInSceneSec);
      });
      if (prevCaption) {
        displayText = prevCaption.text;
        captionProgress = 0.5;
      } else {
        return null;
      }
  ...
  // アニメーションスタイルに応じた値を計算
  // フォールバック時はアニメーションを控えめに
  const animationValues = isUsingFallback
    ? { opacity: 0.9, translateY: 0, translateX: 0, scale: 1 }
    : calculateCaptionAnimation(animationStyle, captionProgress, frame);
  ...
  return (
    ...
    >
      {animationStyle === 'karaoke' && !isUsingFallback ? (
        <KaraokeText
          text={displayText}
          progress={captionProgress}
        />
      ) : animationStyle === 'typewriter' && !isUsingFallback ? (
        <TypewriterCaption
          text={displayText}
          progress={captionProgress}
        />
...
export const AnimatedText: React.FC<AnimatedTextProps> = ({
  ...
  switch (animationType) {
    case 'typewriter':
      return <TypewriterText text={text} durationFrames={durationFrames} sceneStartFrame={sceneStartFrame} style={style} />;
    case 'word_popup':
      return <WordPopupText text={text} durationFrames={durationFrames} sceneStartFrame={sceneStartFrame} style={style} />;
    case 'count_up':
      return <CountUpText text={text} durationFrames={durationFrames} sceneStartFrame={sceneStartFrame} style={style} />;
    case 'slide_up':
      return <SlideUpText text={text} durationFrames={durationFrames} sceneStartFrame={sceneStartFrame} style={style} />;
    case 'fade':
    default:
      return <FadeText text={text} durationFrames={durationFrames} sceneStartFrame={sceneStartFrame} style={style} />;
  }
};
